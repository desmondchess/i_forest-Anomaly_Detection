...
using CSV, ScikitLearn, DataFrames, CairoMakie, PlutoUI, ColorSchemes
...
label_distri = combine(groupby(glass, "label"), "label" => (col -> length(col)) => "# of label distribution")
...
# ╔═╡ 6217bc43-10e2-46b0-a46b-0fb332af3075
md"I think they are"
...
begin
	fig = Figure()
	ax = Axis(fig[1,1], xlabel = "Na weight %", ylabel = "Si weight %", title = "Glass data scatter plot")
	x = glass[:,1]
	y = glass[:,2]
	label = glass[:,3]
	if color_by_label
		for row in 1:nrow(glass)
			if label[row] == "normal"
				scatter!(x[row], y[row], color=colors["normal"], label="Normal")
			else	
				scatter!(x[row], y[row],color=colors["anomaly"], label="Anomaly")
			end
		end
		axislegend(position=:rc, unique = true )
	else
...
feature_matrix = select(glass, Not("label")) 

# ╔═╡ 066bf0c5-435e-43e7-a48b-3dedfc69ee75
feature_matrixX = Matrix(feature_matrix)
...
model = IsolationForest(random_state=0).fit(feature_matrixX)
...
anomaly_score = model.decision_function(feature_matrixX)

# ╔═╡ 5e828dfb-d804-4b6c-a76f-b5e7c5acff90
begin
	local fig = Figure()
	local ax = Axis(fig[1, 1], 
	title="Anomaly score visualization", 
	xlabel="anomaly_score",    
	ylabel="Feature_index")
	density!(anomaly_score; npoints = 500, offset = 0.0, direction = :x)
	vlines!([0],xmin = 0, xmax = 79, width=0.4, color=:red, linestyle = :dashdot)
	fig
	
end

# ╔═╡ 3902cadd-c07c-461d-9314-2c1b3aa8768e
begin
	local fig = Figure()
	local ax = Axis(fig[1, 1], 
	title="Anomaly score visualization", 
...
begin
	new_samples = [14.8 73.1 ; 12.8 73.15]
	model.predict(new_samples)
end

# ╔═╡ b344ddbf-c5aa-4d86-aafc-291d16070ac5
md"Yeah, it does makes sense"
...
# ╔═╡ 6f91be06-8ea4-4682-8db3-8876e36acefb
maximum(feature_matrix[:,2])

# ╔═╡ f6aae642-b2a1-44e2-8f5f-edafdf53be81
begin
	length(collect(12:0.12:18))
	length(collect(71:0.1:76))
end

# ╔═╡ 3f7ba9f6-616c-41b8-a103-bebe8114dc10
begin
	grid_1 = collect(12:0.12:18)
	grid_2 = collect(71:0.1:76)

	grid  = zeros(length(grid_1),length(grid_2))
	anom_grid = []
	grid_index = 0
	for na in 1:length(grid_1)
		for si in 1:length(grid_2)
			push!(anom_grid, [grid_1[na], grid_2[si]])
		end
	end
	#anom_grid
	hm_grid = model.decision_function(anom_grid)
	for na in 1:length(grid_1)
		for si in 1:length(grid_2)
			grid_index +=1
			grid[na,si] = hm_grid[grid_index]
		end
	end
	grid
end

# ╔═╡ 8c83d404-4f91-4350-ad81-44aff0cf4bed
begin
	local fig = Figure()
	local ax = Axis(fig[1,1], 
	xlabel = "Sodium (Na)",
	ylabel = "Silicon (Si)",
	title = "Glass Anomaly Detector Visualization")

	x₁ = glass[:,1]
	y₁ = glass[:,2]
	label₁ = glass[:,3]
	limit = (-maximum(abs.(grid)), maximum(abs.(grid)))

	hm = heatmap!(grid_1,grid_2, grid, colormap=cmap, colorrange=limit)
	Colorbar(fig[1, 2], hm, label="anomaly score")
	contour!(grid_1, grid_2, grid, line_width=2, levels = [0.0], color=:black)
	...
	xlims!(minimum(grid_1), maximum(grid_1))
	ylims!(minimum(grid_2), maximum(grid_2))
	fig
end
...
begin
	y_pred = model.decision_function(feature_matrixX)
	target_predict = Int.(y_pred .>= 0.0)
end

# ╔═╡ 5b782edf-280d-4f60-912f-39590b1c0e0c
ground_truth = Int.(glass[:,3] .== "normal")
...
begin
	...
	conf_matrix = confusion_matrix(ground_truth,target_predict)
end

# ╔═╡ 02e2eaa0-b2f3-4f8a-8e7b-f623db9bd845
begin
	class_list = ["anomalous","normal"]
	viz_confusion_matrix(conf_matrix,class_list)
end
...
md"6 false alarms.
While, 4 passed through the iForest without alarm"
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "935ee089ec579766556d9c4e68fe4548253d578e"
